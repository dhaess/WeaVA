...
const Histogram = ({dimensions, id}) => {
    ...
    useEffect(() => {
        ...
        if (data.length !== 0) {
            ...
            const histData = binDataRange(data.map(e => e.timestamp));
    ...
    }, [data, binCount, height, margin.bottom, margin.left, margin.right, margin.top, width, dispatch, dragStart, timeRange]);
